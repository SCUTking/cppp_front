...
import homeworkSummary from '../learn/homework/homeworkSummary.vue';
import homeworkCard from '../learn/homework/homeworkCard.vue';
...
let courseId, chapterId, chapterType, ccId, paperId;
onMounted(() => {
  courseId = proxy.$route.query.courseId;
  chapterId = proxy.$route.query.chapterId;
  chapterType = proxy.$route.query.chapterType;
...
const getPaperWaitingMsg = async () => {
  const res = await proxy.$api.getPaperWaitingMsg({
    paperId: proxy.$route.query.paperId,
  });
